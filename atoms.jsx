/* AETHEL atoms — small reusable bits */
...
// Animated counter that fires when in view
...
// SVG sigil for lineages — simple geometric mark
function LineageSigil({ id }) {
  const map = {
    ignaris: (
      <g>
...
// Decorative cosmic ring (hero sigil)
function CosmicRing() {
  return (
...
// Typing-text effect — fires when in view, supports caret + multiple lines
function Typer({ text, speed = 38, startDelay = 200, className, caret = true, onDone }) {
  const ref = useRef(null);
...
// Parallax hook — moves an element with scroll
...
Object.assign(window, { useReveal, Counter, ParticleBg, LineageSigil, SealSigil, CosmicRing, CrownSigil, Typer, ScrollProgress, useParallax, SocialIcon });
